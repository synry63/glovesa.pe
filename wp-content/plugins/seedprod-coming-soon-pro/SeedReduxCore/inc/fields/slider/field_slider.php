...
if ( ! class_exists( 'SeedReduxFramework_slider' ) ) {
    class SeedReduxFramework_slider {
         ...
         *
         * @since SeedRedux_Framework 3.1.8
         */
        function clean() {
        ...
        function enqueue() {
            ...
            // wp_enqueue_style(
            //     'seedredux-nouislider-css',
            //     SeedReduxFramework::$_url . 'inc/fields/slider/vendor/nouislider/seedredux.jquery.nouislider.css',
            //     array(),
            //     '5.0.0',
            //     'all'
            // );
            ...
            // wp_register_script(
            //     'seedredux-nouislider-js',
            //     SeedReduxFramework::$_url . 'inc/fields/slider/vendor/nouislider/seedredux.jquery.nouislider' . $min . '.js',
            //     array( 'jquery' ),
            //     '5.0.0',
            //     true
            // );
            ...
            // wp_enqueue_script(
            //     'seedredux-field-slider-js',
            //     SeedReduxFramework::$_url . 'inc/fields/slider/field_slider' . $min . '.js',
            //     array( 'jquery', 'seedredux-nouislider-js', 'seedredux-js', 'select2-js' ),
            //     time(),
            //     true
            // );
            ...
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                    'seedredux-field-slider-css',
                    SeedReduxFramework::$_url . 'inc/fields/slider/field_slider.css',
                    array(),
                    time(),
                    'all'
                );
        ...
        function render() {
            ...
            if ( $this->display_text == $this->field['display_value'] ) {
                ...
                echo '<input type="text"
                         ...
                         id="' . $idOne . '"
                         value="' . $valOne . '"
                         class="seedredux-slider-input seedredux-slider-input-one-' . $fieldID . ' ' . $this->field['class'] . '"/>';
            ...
            // LABEL output
                ...
                echo '<div class="seedredux-slider-label' . $labelNum . '"
                       id="seedredux-slider-label-one-' . $fieldID . '"
                       name="' . $nameOne . '">
                  </div>';
            ...
            // SELECT output
                ...
                echo '<select class="seedredux-slider-select-one seedredux-slider-select-one-' . $fieldID . ' ' . $this->field['class'] . '"
                          name="' . $nameOne . '"
                          id="' . $idOne . '">
                 </select>';
            }
            ...
            echo 
            '<div
                class="seedredux-slider-container ' . $this->field['class'] . '"
                id="' . $fieldID . '"
                data-id="' . $fieldID . '"
            ...
            if ( true == $twoHandles ) {
                ...
                if ( true == $showInput ) {
                    echo '<input type="text"
                             ...
                             id="' . $idTwo . '"
                             value="' . $valTwo . '"
                             class="seedredux-slider-input seedredux-slider-input-two-' . $fieldID . ' ' . $this->field['class'] . '"/>';
                }

                // LABEL
                if ( true == $showLabel ) {
                    echo '<div class="seedredux-slider-label-two"
                           id="seedredux-slider-label-two-' . $fieldID . '"
                           name="' . $nameTwo . '">
                      </div>';
                }
                ...
                // SELECT
                if ( true == $showSelect ) {
                    echo '<select class="seedredux-slider-select-two seedredux-slider-select-two-' . $fieldID . ' ' . $this->field['class'] . '"
                              name="' . $nameTwo . '"
                              id="' . $idTwo . '">
                     </select>';
            ...
            if ( $this->display_none == $this->field['display_value'] || $this->display_label == $this->field['display_value'] ) {
                echo '<input type="hidden"
                         class="seedredux-slider-value-one-' . $fieldID . ' ' . $this->field['class'] . '"
                         name="' . $nameOne . '"
                         id="' . $idOne . '"
                         value="' . $valOne . '"/>';
                ...
                if ( true == $twoHandles ) {
                    echo '<input type="hidden"
                             class="seedredux-slider-value-two-' . $fieldID . ' ' . $this->field['class'] . '"
                             name="' . $nameTwo . '"
                             id="' . $idTwo . '"
                             value="' . $valTwo . '"/>';
                }
