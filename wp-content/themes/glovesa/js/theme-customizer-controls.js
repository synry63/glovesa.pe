...
;(function ($) {
	...
	$( window ).load( function() {
		var customizePreview = $( '#customize-preview' ),
			...
			settingContainerWrapper = $( '#customize-control-ref_container_wrapper input' ),
			SettingCSSAll =  $( '#customize-control-ref_wc_styles, #customize-control-ref_theme_styles, #customize-control-ref_color_styles' ).find( 'input' ),
			reloadAfterRefresh;


		settingContainerWrapper.on('change', function(){
			var iframe = $( '#customize-preview iframe' ).contents(),
				iframeContainerWrapper = iframe.find('section.container-main[role="main"]');
			if(settingContainerWrapper.is(':checked')){
				iframeContainerWrapper.addClass('container');
			} else {
				iframeContainerWrapper.removeClass('container');
			}
		...
		if( primaryColorControl.length && ! settingPrimaryColor.attr( 'disabled' ) ) {
			...
			primaryColorInput.on( 'change', function() {
				...
				SettingCSSAll.attr( 'disabled', true );
				...
				$.ajax( {
					...
					success : function ( css ) {
						...
						SettingCSSAll.removeAttr( 'disabled' );
					}
				} );
